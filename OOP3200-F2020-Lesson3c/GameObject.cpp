...
GameObject::GameObject(std::string name, const int id, const float x, const float y) : m_id(id), m_name(std::move(name)), m_position(Vector2D<float>(x, y))
{
}

GameObject::GameObject(std::string name, const int id, const Vector2D<float>& position) : m_id(id), m_name(std::move(name)), m_position(position)
{
}
